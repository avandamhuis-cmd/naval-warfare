...
export class Musket {
  constructor(scene, camera, player) {
    ...
    window.addEventListener('mousedown', (e) => {
      if (this.equipped) {
        ...
        if (e.button === 2) this.aiming = true; // Right click
      }
    });
  ...
  toggleEquip() {
    ...
    this.musketGroup.visible = this.equipped;
  }
  ...
  reload() {
    ...
    // Simple reload animation (moves musket slightly down and up)
  ...
  update(bulletsTarget = []) {
    ...
    this.scene.children.forEach((child) => {
      if (child.userData && child.userData.velocity) {
        ...
        bulletsTarget.forEach(target => {
          if (child.position.distanceTo(target.position) < 1) {
    ...
    // Update TWEEN animations for reload
